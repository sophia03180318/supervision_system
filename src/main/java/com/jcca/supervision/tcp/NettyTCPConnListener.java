 ...
 * @author: sophia
 */
public class NettyTCPConnListener implements ChannelFutureListener {

    private Logger logger = LoggerFactory.getLogger(getClass());

    private NettyTCPClient nettyTCPClient;

    public NettyTCPConnListener(NettyTCPClient nettyTCPClient) {
    ...
    public void operationComplete(ChannelFuture future) {
        if (!future.isSuccess()) {
            ...
            logger.error(LogUtil.buildLog(tcpConfig.getTcpIp() + ":" + tcpConfig.getTcpPort(),
                    "TCP网络不通", (nettyTCPClient.getCount()) + "秒后重新连接"));
